...
class side:
    ...
    def render(self):
        ...
        glEnd();
    def get_center(self):
        return self.p1+(self.p1-self.p2)*0.5
    def __str__(self):
        return "side {0} - {1} - {2} - {3} ".format(str(self.p1),str(self.p2),str(self.p3),str(self.p4))
    def __repr__(self):
        return str(self)
...
class OneBox:
    ...
    def render_for_point(self,p):
        s=sorted(self.lst,key=lambda so:(so.get_center()-p)())
        for i in s:
            i.render();
        #print(s,self.lst)
